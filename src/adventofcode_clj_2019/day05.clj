...
(defn execute [input tape]
  (letfn [(mkop [tape i op f1 f2] (assoc tape (tape (+ i 3))
    ...
    (loop [tape tape, i 0, out []]
      (let [m (partial mode tape)]
        (match (parse-opcode (get tape i))
          [_,_,9,9] out
          [a,b,0,1] (recur (mkop tape i + (m b) (m a)) (+ i 4) out)
          [a,b,0,2] (recur (mkop tape i * (m b) (m a)) (+ i 4) out)
          [_,_,0,3] (recur (assoc tape (tape (inc i)) input) (+ i 2) out)
          [_,_,0,4] (recur tape (+ i 2) (conj out ((comp tape tape) (inc i))))
          [a,b,0,5] (recur tape (newi tape i #(not= 0 %) (m b) (m a)) out)
          [a,b,0,6] (recur tape (newi tape i zero? (m b) (m a)) out)
          [a,b,0,7] (recur (setv tape i < (m b) (m a)) (+ i 4) out)
          [a,b,0,8] (recur (setv tape i = (m b) (m a)) (+ i 4) out))))))
